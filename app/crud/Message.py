...
def verify_users_are_matched(user_id1: str, user_id2: str) -> bool:
    """Verify that two users are matched (security check)"""
    session = get_db()
    try:
        query = """
        MATCH (u1:User {user_id: $user_id1})-[:MATCHES]-(u2:User {user_id: $user_id2})
        RETURN count(*) as match_count
        """
        result = session.run(query, {"user_id1": user_id1, "user_id2": user_id2}).single()
...
def create_message(sender_id: str, receiver_id: str, content: str, match_id: str = None):
    session = get_db()
    try:
        # SECURITY: Verify users are matched before allowing message
        if not verify_users_are_matched(sender_id, receiver_id):
            raise ValueError("Users must be matched to send messages")
...
def mark_conversation_as_read(user_id: str, other_user_id: str):
    """Mark all messages from other_user as read by current user"""
    session = get_db()
    try:
        query = """
        MATCH (m:Message {sender_id: $other_user_id, receiver_id: $user_id, is_read: false})
        SET m.is_read = true, m.read_at = $read_at
...
def mark_message_as_delivered(message_id: str):
    """Mark a message as delivered"""
    session = get_db()
    try:
        query = """
        MATCH (m:Message {message_id: $message_id})
        SET m.delivered = true, m.delivered_at = $delivered_at
        RETURN m
        """
        result = session.run(query, {
            "message_id": message_id,
